...
fig_reduce.suptitle('Times for Reduce Ones EVA Program')

# REDUCE FILES
for i in range(len(reduce_files)):
	distance_file = reduce_files[i]
	...
	df = pd.read_csv(distance_file).drop(columns=['sim'])
	if i == 0:
...
fig_distances.suptitle('Times for Compute Distances EVA Program')

# DISTANCES FILES
for i in range(len(distances_files)):
	distance_file = distances_files[i]
...
fig_mse.suptitle('MSE Scores of EVA Programs')
